...
- P2 按包围矩 IoU、中心位移、颜色差、面积比和 32x32 形状指纹作硬门禁与代价。对每个有限联通竞争组使用全局最小代价一对一分配；节点数超过 64 的组不关联，宁可显式新建候选也不拿不可审核的贪心结果冒充确认。未确认候选的关联还受最大代价门禁，默认 `200000/1000000`。轨迹的可靠关联参考只在代价不高于该门槛时更新；高代价当前观测不再逐帧拖动后续关联基准。
- 实验开关 `--allow-tentative-growth-association` 仅处理目标进入/退出画面时包围矩快速增长或收缩：只允许 `Tentative` 候选，中心距离、面积比、较小包围矩覆盖率、颜色差和最小像素数均需通过门禁。增长特例只保持相机候选编号并重置确认累计；增长结束后必须重新取得完整的连续普通命中，不能靠特例晋级 `Active`。默认关闭；当前真实 T3 开发片段显示全阶段候选号种类 `9 -> 4`、相邻换号 `9 -> 4`，但 Active 命中帧 `19 -> 18`；静态 `600x3` 离线跟踪回归零触发且生命周期事件不变。该结果仍缺独立 T3 留出片段，不是默认策略或现实身份验证。
...
- `run_cluster_control_gate.py` 可用完整 `PCS.RawSequence/1` 或相机运行影子门禁，记录输入清单哈希、Git修订、配置、逐帧结果和 p50/p95/p99/max，但当前不以影子耗时判定控制语义通过。真实静态120帧语义门禁通过，p95约1419ms，故不能晋级实时实现。
- `evaluate_cluster_scenario_capture.py` 在 T3/T7 算法门禁前验证采集内容：T3 区分局部变化与整帧变化，T7 只从原始彩图暗区和原始深度0值的交集判断是否录到孔洞压力。门槛按画面比例归一化；输出明确保留人工复核和现实真值未证明状态。`capture_and_verify_cluster_scenario.py` 把新采、内容门禁和控制门禁串成一次不可覆盖运行。
- `render_cluster_scenario_review.py` 为每段采集生成最多6组原图和证据叠加图及 SHA256 清单。叠加色只是算法掩码，不是目标身份或孔洞真值；视觉复核状态不会被脚本自动改成通过。

...
相同真实静态材料的 `120x3` 结果确定一致：非触边闭合候选 IoU p50 从 `84.7244%` 提升到 `99.3151%`，中心位移 p95 从 `67.5104px` 降到 `1.5811px`，静态生命周期事件从45次降到0次；三次结构门禁通过。该结果仍是静态 replay probe：C++ 源处理 p95 三轮约 `121.9/128.2/94.0ms`，完整 Python 影子链尚远高于100ms。转换器现按 `retained_cluster_pixels` 前置过滤，不再为不可保留的小碎片生成完整簇包；单次影子链 elapsed p95 从约 `2568ms` 降到 `1363ms`，但仍不具备实时生产能力。默认 `允许同彩图区域跨缺测全局合并=true` 和默认 `深度主导` 均未改变。

...
`capture_and_verify_cluster_scenario.py` 串联新采 `PCS.RawSequence/1`、上述内容门禁和簇级控制门禁，保存原始清单、内容指标、控制结果和顶层决策。每完成一个阶段就更新顶层清单；后续控制门禁失败时，已经完成的采集、内容门禁和视觉复核材料不再被写回为 `null`。60帧合成局部运动端到端测试及强制控制失败证据保留测试已通过。真实 T3/T7 必须省略 `--replay-source` 从相机新采；旧 `d455_directory_replay_v1` 数据缺少完整标定与逐帧时间合同，不能转换后冒充本门禁通过。
...
真实静态序列的首轮600帧暴露了两类长期问题：已确认轨迹会被高代价小碎片逐帧拖离可靠主体；单包分割缺失会立即误报遮挡/重现。加入可靠关联参考和2包遮挡确认、3包丢失期限后，同一 `PCS.RawSequence/1` 顺序运行3次，每次600帧，包验证、规范化确定性、增量重建、候选存在和静态生命周期门禁全部通过；`static_lifecycle_event_count=0`、`run_max_active_tracks=[8,8,8]`。三轮仍有363次未确认候选移除、381次新增和2907次帧内局部号重用诊断，底层分割噪声没有消失，帧内号重用也不等于跨帧 ID 切换。结合 T9 合成故障注入，目前只完成了 P2 Python 影子链的协议、重建、生命周期和故障恢复门禁；T2 新增的32x32填充轮廓 IoU 与中心抖动门禁尚未用当前协议的真实600帧x3重新退出，因此不得再把 P2 整体写成完成。真实动态发现、物理断连和 C++ 实时恢复也仍未验证。

`evaluate_cluster_stability.py` 现在从无损轮廓链重建居中32x32填充轮廓，输出 `track_geometry_metrics.csv` 和 `geometry_pairs.csv`。统计明确分开材料真实更新与心跳/缓存状态复用；主候选按材料更新数、发布持续帧数和像素中位数确定，同时保留全部轨迹明细。静态主候选轮廓 IoU p50 的目标为99%，中心位移 p95 本轮只建立基线。完全相同的合成静态流已得到100% IoU；真实600帧x3需按新协议重跑，旧包缺少后续冻结字段，不能跳过严格读回直接补算。
...
2026-09-21 的 A2 探针为实验性轮廓模式增加 `深度拆分最小支持像素=32` 和 `跨颜色合并最小连续边界像素=8`：小深度碎片合并到同一彩图区域内最近的受支持深度层，跨彩图区域则要求连续深度边界达到门槛才允许合并。协议合成反例为 `28/28` 通过，但同一真实静态材料的 `120x3` 门禁仍失败：主候选相邻 32x32 IoU p50=`97.5258%`、不同值 `118/120`、中心位移 p95=`0px`。读回还表明该主候选是触边的全画面缺深度区域，不是已证明的前景存在。所以下一步是前景/开放边界/背景账本候选分层和无深度内部归属，不继续盲调深度阈值；默认聚簇模式仍为 `深度主导`。

后续 A2 分层修正已经把评估对象拆成 `全画面账本区域`、`触边开放候选` 和 `非触边闭合候选`。只有真实运行中存在非触边闭合候选时，99% 轮廓门槛才作用于该层；纯全画面合成输入明确回退到 coverage-main 门禁，结构分层不推断背景或现实身份。逐帧诊断证明跨颜色合并为 0，而同彩图区域仅凭平均深度相近的跨缺测全局合并每帧发生 29–54 次。新 probe 可用 `--disable-global-missing-depth-merge` 关闭该规则；小于32像素的深度种子只有与受支持层存在直接边界且深度相容时才合并，最多4像素的不相容噪点可被吸收，非局部碎片保持独立。

相同真实静态材料的 `120x3` 结果确定一致：非触边闭合候选 IoU p50 从 `84.7244%` 提升到 `99.3151%`，中心位移 p95 从 `67.5104px` 降到 `1.5811px`，静态生命周期事件从45次降到0次；三次结构门禁通过。源端随后增加分阶段耗时诊断，并做两项不改变聚簇判定的扫描优化：深度种子建立时直接登记所属彩图区域，同一次邻接扫描同时采集同色与跨颜色边界；彩图 flood 完成后按区域像素数一次性预留成员空间。复跑同一 `120x3` 后，三轮 C++ `处理毫秒` p95 为 `88.97/83.84/91.64ms`，`分区毫秒` p95 为 `46.72/46.72/53.83ms`，静态源处理短门禁通过且结构结果完全不变。`packet_metrics.csv` 现在同时记录配准、颜色转换、分区、补全、轮廓生成及分区内部八个阶段耗时。

转换器按 `retained_cluster_pixels` 前置过滤，不再为不可保留的小碎片生成完整簇包；但完整 Python 影子链本次三轮 elapsed p95 仍为 `1016.52/1022.07/1056.81ms`，不具备生产实时能力。以上证据只覆盖固定真实静态材料的120帧短回放，不证明 `600x3`、动态跟踪、世界身份或绝对深度准确性。默认 `允许同彩图区域跨缺测全局合并=true` 和默认 `深度主导` 均未改变。

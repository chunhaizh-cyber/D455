...
def main():
    ...
    try:
        ...
        tiny_outlier = depth.copy()
        tiny_outlier[12, 16] = 2000
        ...
        run("contour_first_merges_painted_continuous_surface", lambda: geometry_case("contour_paint", patterned, depth,
            lambda m, a: check(len(m["簇目录"]) == 1 and m["指标"]["跨颜色连续深度合并数"] >= 1,
                              "Supported continuous color boundary split one surface"), configuration={"聚簇模式": "轮廓主导"}))
        false_bridge = step.copy()
        false_bridge[12, 16] = 1000

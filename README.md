...
P0/P1/P2 已建立 `eval/visual_evolution/` 记录契约、历史样例、校验器和 `scripts\derive_visual_requirements.py` 需求候选生成器。可运行 `python scripts\validate_visual_evolution_records.py --root eval\visual_evolution --check-evidence --analysis-runs analysis_runs` 检查本地证据；需求生成器默认只输出候选，只有显式 `--register` 且达到持续性门槛才登记。当前仍未实现任务调度或方法自动晋级。

...
2026-09-20 形成后续簇级供包和验收方案：用统一的 `PCS.ClusterObservation/1` 承载扫描、观察和跟踪负载，相机只建立会话内短期簇跟踪候选，不确认现实存在身份。P0 已实现严格独立读回、材料 SHA256、规范 8 方向轮廓压缩、距离证据边界和全量/增量/心跳结构校验；P1 已实现一个独立的 `PCS.Observation/1 -> FullSnapshot + Scan` 转换器；P2 已实现离线短期簇跟踪、遮挡/墓碑事件、增量发布和状态重建；P3 Python 影子桥已将连续来源帧串成首帧全量、后续增量、运行清单和审计 CSV，并在三帧合成静态回放中验证稳定候选号与状态重建。该桥不拥有相机，唯一设备所有者仍为 C++ 服务；它不是 C++ 实时供包接线或调度能力。P2/P3 尚未经真实动态回放、心跳生产、调度指令、姿态补偿或精确三维升级验收。旧 `datasets/*` 录制缺少 `PCS.RawSequence/1` 的完整标定和逐帧时间域，不能伪造为动态跟踪证据。现有转换会将所有距离标为 `UnknownDistance`，并保留深度证据分账，防止把“范围内观测”伪造为 `PreciseDepth3D`。详见 [簇级扫描观察跟踪稳定供包测试方案](资料/20260920_D455簇级扫描观察跟踪稳定供包测试方案_v0.1.md) 与 [簇级协议](PixelClusterSensor/PROTOCOL.md)。

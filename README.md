...
2026-09-20 形成后续簇级供包和验收方案：拟议用统一的 `PCS.ClusterObservation/1` 分别承载扫描、观察和跟踪负载，相机只建立会话内短期簇跟踪候选，不确认现实存在身份。方案冻结包头、簇记录、距离降级、全量/增量/心跳重建、有界详细材料句柄、扫描/观察/跟踪指令、分阶段测试和硬门禁。当前仅是待实现规格，不改变 `PCS.Observation/1` 默认供包行为；详见 [簇级扫描观察跟踪稳定供包测试方案](资料/20260920_D455簇级扫描观察跟踪稳定供包测试方案_v0.1.md)。

...
P0/P1/P2/P3 已建立 `eval/visual_evolution/` 记录契约、历史样例、校验器、需求候选生成器、适用方法查询和单目标任务筹办脚本。当前任务在缺少独立留出集时会明确标记 `blocked_missing_holdout`，probe-only 方法默认不会作为生产基线；任务驱动的 replay 元数据传递和自动晋级仍未实现。

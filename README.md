...
真实 `hand_occlusion_reappear` 已完成第一段 120 帧本地采集，并通过 reviewed manifest 校验；数据集仍位于本机 `datasets/hand_occlusion_reappear`，不进入 Git。`replay_hand_occlusion_reappear_real_001` 跑了 `candidate_0030`、`candidate_0034`、`candidate_0035`：0030 以 `score=92.562`、p95=76.2424ms 保持真实遮挡场景最佳；0035 pass 但 p95=98.773ms、max=132.654ms、over-100ms=5；0034 因 p95=101.6394ms hard fail。0035 的 G3 red-line 为 pass：`runtime_roi_stereo_dropped_count=0`、`missing_baseline_region_count=0`、`dropped_stereo_region_count=0`、`contour_lost_event_count=0`。但这次真实场景中 0035 的 ROI 候选面积 p95=303744px，7 次被 `rejected_large`，所以它没有证明“局部 ROI direct-build”在真实遮挡中有效，仍保持 probe-only。

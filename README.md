...
2026-09-20 形成后续簇级供包和验收方案：用统一的 `PCS.ClusterObservation/1` 承载扫描、观察和跟踪负载，相机只建立会话内短期簇跟踪候选，不确认现实存在身份。P0 已实现严格独立读回、材料 SHA256、规范 8 方向轮廓压缩、距离证据边界和全量/增量/心跳结构校验；P1 已实现一个独立的 `PCS.Observation/1 -> FullSnapshot + Scan` 转换器。它尚未接入实时供包服务，没有跨帧跟踪、增量/心跳生产、调度指令、姿态补偿或精确三维升级。现有转换会将所有距离标为 `UnknownDistance`，并保留深度证据分账，防止把“范围内观测”伪造为 `PreciseDepth3D`。详见 [簇级扫描观察跟踪稳定供包测试方案](资料/20260920_D455簇级扫描观察跟踪稳定供包测试方案_v0.1.md) 与 [簇级协议](PixelClusterSensor/PROTOCOL.md)。

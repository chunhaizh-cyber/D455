...
2026-07-10 已完成 Auto1→Locked1→Auto2→Locked2 各600帧交叉测试。两次 Auto 的左右IR均值约为53/60，两次 Locked 约为162/161，说明亮度工作点跃迁随控制模式重复出现；归一化后，两次 Locked 的 IR 均值 CV 约0.12%，低于 Auto 的0.47%/0.64%，归一化强度和梯度差也一致下降。锁定控制同时改善了本场景的彩图、深度有效性和深度数值波动，但不能据此宣布所有场景已稳定：Locked2 为接收600个合格帧组拒绝了637个不同步候选，且设备不提供逐帧曝光元数据。后续实时采集会把同步拒绝拆成流未前进、左右IR帧号不一致和时间跨度超限三类。

...
- 四个窗口显示：
  ...
  - `D455 stable contour information`：以左红外灰度图为背景，显示稳定基准点、稳定点支撑的候选轮廓、历史确认后的稳定轮廓和当前稳定评分。
  - `D455 stable contour color mosaic`：只显示稳定轮廓内部的原始彩图；P5 测试分支默认会在稳定轮廓邻域内严格使用 RGB 轮廓补齐断口，补齐候选必须和原稳定轮廓差异很小、且不碰到其他稳定轮廓隔离带才采用，其他区域保持黑色未知背景。
  - `D455 black-area color view`：单独显示第三窗口未知背景对应的原始彩图，稳定/补齐轮廓内部保持黑色。
...
P5 测试分支默认启用严格 RGB 轮廓补齐显示：只在稳定轮廓的小邻域里用彩图生成候选补齐轮廓；如果候选和原稳定轮廓的 IoU、面积变化、中心偏移都在阈值内，且候选不会进入其他稳定轮廓的隔离带，第三/第四窗口才采用彩图补齐后的轮廓，否则继续使用原稳定轮廓。未知背景不会被强行分配给任何前景轮廓，也不会为了补齐而把两个轮廓融合。需要临时退回原第三窗口裁剪口径时：

```powershell
.\x64\Release\D455.exe --no-color-contour-completion
```
...
P2 cue selection 默认启用。它在稳定锚点候选进入历史 tracker 前做轻量择机：深度边、深度确认灰度边、强稳定锚点任一可信则保留；只有明显灰度纹理边占主导且深度/灰度确认和锚点都弱的候选会被挡住，已有稳定 track 会由历史保持机制短时接管。第二窗口底部显示 cue 统计，CSV 会同步记录 cue 输入、保留、纹理拒绝和各类接受原因：

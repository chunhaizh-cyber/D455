...
缓存刷新现在会进入可度量输出：D455 在 `profile.csv` 追加 `color_contour_refreshed`、`color_contour_cache_reused`、`color_contour_refresh_motion`、`color_contour_refresh_unknown_spike`、`color_contour_refresh_far_loss`、`color_contour_motion_delta_percent`、ROI 刷新/候选面积/拒绝原因和 stereo reuse 相关字段；converter 会把它们写入 `frame_metrics.csv`，并在刷新帧写 `events.csv` 的 `color_contour_refresh` 事件。`run_score.json` 也汇总 refresh/cache/reuse/ROI 计数，后续 motion gate 不再只靠 p95 和 far_score 间接推断触发策略是否有效。

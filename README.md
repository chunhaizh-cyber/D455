...
缓存刷新现在会进入可度量输出：D455 在 `profile.csv` 追加 `color_contour_refreshed`、`color_contour_cache_reused`、`color_contour_refresh_motion`、`color_contour_refresh_unknown_spike`、`color_contour_refresh_far_loss`、`color_contour_motion_delta_percent`、ROI 刷新/候选面积/拒绝原因、ROI 内刷新 region 数、ROI stereo reuse/failed 数和 preserved stereo 数；converter 会把它们写入 `frame_metrics.csv`，并在刷新帧写 `events.csv` 的 `color_contour_refresh` 事件。`run_score.json` 也汇总 refresh/cache/reuse/ROI 计数，后续 motion gate 不再只靠 p95 和 far_score 间接推断触发策略是否有效。
...
G1/G2 已拆成独立门禁：G1=`roi_stereo_g1_preservation_pass`，要求 ROI 触发、没有 `far_stereo_failed`，并存在 preserved/reused/built stereo evidence；G2=`roi_stereo_g2_rebuild_pass`，要求 ROI 新 region 没有 stereo failed，并且 `roi_stereo_reuse_count > 0` 或 `roi_stereo_built_count > 0`。为验证 G2，`generate_local_motion_roi_probe.py` 新增 `--right-ir-shift-x`，可让 synthetic 目标在右 IR 中产生正视差；`local_motion_roi_stereo_probe` 使用 `--right-ir-shift-x=-30` 和 1200mm 深度生成。`candidate_0034` 是 G2 direct-build 探针：关闭 async/cache reuse，让 ROI 同步路径直接跑 stereo。`replay_local_motion_roi_stereo_probe_001` 显示：0033 通过 G2，路径是新 ROI region 复用 cached stereo evidence（`roi_stereo_reuse_count=6`、failed=0）；0034 direct build 部分有效（`roi_stereo_built_count=4`），但仍有 `roi_stereo_failed_count=4`，所以直接 ROI stereo rebuild 还没有闭环。0032/0033/0034 都是 ROI probe，不替代 `candidate_0030` 的 slow-pan best。
...
python scripts\run_batch.py --candidates configs\candidates\round_001 --candidate-id candidate_0033 --candidate-id candidate_0034 --cases eval\cases.yaml --case-id local_motion_roi_stereo_probe --max-frames-override=120 --analysis-export-every-n=15 --ignore-first-n-frames=15 --validate-replay --require-replay-ir --require-reviewed-manifest --execute
```

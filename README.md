...
真实慢速平移数据 `datasets/slow_pan_far_object` 已完成本地采集，包含 180 帧 color/depth16/left IR/right IR，并已按 case contract finalized/reviewed；检查视频为本地 `recordings/slow_pan_far_object_color_20260706_1549.mp4`，数据集和视频不进入 Git。`replay_motion_slow_pan_001` 跑了 0013、0014、0015、0017、0018-0029。评分器现在用完整 `profile.csv` 计算性能 p95，用导出帧 `frame_metrics.csv` 计算分割质量，并在 leaderboard 保留导出采样 p95、最大帧耗时、超 100ms 帧数作为顿挫风险指标。`candidate_0028` 是当前 slow-pan 阶段赢家：p95 69.0991ms、max 98.105ms、超 100ms 帧数为 0、far_score 10.0；它基于 0024/0027 的粗粒度路线，增加 `--color-contour-refresh-min-gap-frames=15`，并记录 7 次 cooldown skip。该结果已进入 `configs/best/best_replay_motion_slow_pan.json`，但它仍是 coarse slow-pan bucket best，不是 overall best。

第一版多线程实现为 `--async-color-contour-refresh`：启动帧仍同步建立首个 cache；之后 motion/interval/unknown/far-loss 触发只在后台 worker 空闲时提交彩图轮廓刷新任务，主线程继续使用旧 cache；worker 完成后按 bbox IoU/中心距离转移 cached stereo 粗距并切换新 cache。`--async-color-contour-low-priority` 会在 Windows 下把 worker 线程降为 `THREAD_PRIORITY_BELOW_NORMAL`；0026 实测没有改善 slow-pan p95/尖峰，只保留为可控开关。`--color-contour-refresh-min-gap-frames=N` 会跳过过近的非 startup 刷新请求并复用 cache，profile/leaderboard 记录 `color_contour_refresh_cooldown_skipped_count`。score 同时保留全帧 `color_contour_async_worker_ms_p95` 和只统计非零 worker 帧的 `color_contour_async_worker_ms_positive_p95`；0028 的 positive worker p95 为 23.2298ms，说明 coarse async worker 本身已不重，剩余风险主要是粗粒度质量与缓存滞后。下一步仍应优先采集/跑 `hand_occlusion_reappear`，再做 ROI 局部刷新或分片刷新。

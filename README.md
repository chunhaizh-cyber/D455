...
2026-09-08 新增 `PixelClusterSensor/PixelClusterSensor.vcxproj`，在 D455 目录内独立实现“原始RGBD -> 像素簇观测帧 -> 自我读取”的供包端，已登记到 `D455.slnx`，不改变现有 `D455.exe` 的默认参数、观察/显示链路和best候选。新链以真实适用标定做配准，输出完整颜色、像素归属、无简化轮廓、当前观测深度及独立的颜色引导补全层；不输出体素、不直接确认存在、不把插值或历史信息计作实测深度。
...
2026-09-20 形成后续簇级供包和验收方案：用统一的 `PCS.ClusterObservation/1` 承载扫描、观察和跟踪负载，相机只建立会话内短期簇跟踪候选，不确认现实存在身份。P0 已实现严格独立读回、材料 SHA256、规范 8 方向轮廓压缩、距离证据边界和全量/增量/心跳结构校验；P1 已实现一个独立的 `PCS.Observation/1 -> FullSnapshot + Scan` 转换器；P2 已实现离线短期簇跟踪、遮挡/墓碑事件、增量发布和状态重建，并将新候选准入、未确认可靠关联、已确认轨迹低阈值保留分成三个门禁，防止小碎片接力形成伪稳定轨迹；P3 Python 影子桥已将连续来源帧串成首帧全量、后续增量、运行清单和审计 CSV。新增受限 `PCS.RawSequence/1` 导出器可由真实 RGBD 观察包保存原始彩图/深度、实际标定和逐帧时间合同，解决动态回放不能凭旧录制补造标定和时间的问题；当前没有导出 IR/IMU，仍不能验证双目 IR 粗距或姿态补偿。该桥不拥有相机，唯一设备所有者仍为 C++ 服务；它不是 C++ 实时供包接线或调度能力。P2/P3 尚未经真实动态回放、心跳生产、调度指令、姿态补偿或精确三维升级验收。现有转换会将所有距离标为 `UnknownDistance`，并保留深度证据分账，防止把“范围内观测”伪造为 `PreciseDepth3D`。详见 [簇级扫描观察跟踪稳定供包测试方案](资料/20260920_D455簇级扫描观察跟踪稳定供包测试方案_v0.1.md) 与 [簇级协议](PixelClusterSensor/PROTOCOL.md)。

2026-09-21 根据既有候选和轮廓报告，冻结下一代聚簇路线为“彩图闭合轮廓主导、深度边界约束拆分、簇内深度赋值与短时稳定、8/16/32 分级轮廓联合时序关联”。`candidate_0030` 继续作为旧主程序可运行基线；`candidate_0048` 只在核心稳定后作为差异扫描、缓存和局部并发效率层。当前 `PixelClusterSensor` 深度主导路径保留为协议对照，实验性轮廓主导路径也不直接晋级默认。新路线按轮廓候选、深度约束、真实孔洞、簇内深度时序、轮廓关联、供包整合和效率调度分别验收，完整方案见 [轮廓主导聚簇实现方案](资料/20260921_D455轮廓主导深度约束时序稳定聚簇实现方案_v0.1.md)。

首个 A2 证据门控探针已实现并验证合成反例，但真实静态 `120x3` 中主候选轮廓 IoU p50 仍为 `97.5258%`，没有达到 `99%`；一次彩图中值去噪筛选同样无效并已撤销。读回确认当前评估选中的最大簇是触边、覆盖全画面且主要由缺深度像素构成的 `UnknownDistance` 区域，不能当成主要存在。后续实现因此增加候选分层：分别处理前景闭合轮廓、触边开放区域和背景/全画面账本，并在指定前景目标上验证无深度内部归属；在此之前不运行完整 A2 `600x3`，也不改变 `candidate_0030` 或默认深度主导路径。

A2 后续实验证明主要错误不是跨颜色边界，而是同一彩图区域内把空间不相邻、仅平均深度相近的种子跨缺测全局合并。关闭该规则并要求小种子具备局部边界证据后，同一真实静态 `120x3` 的非触边闭合候选 IoU p50 达 `99.3151%`、中心位移 p95 为 `1.5811px`、生命周期事件为0，三次确定性与结构门禁通过。全画面账本区域仍单独报告且不作为存在轮廓门禁。随后把深度种子所属彩图区域、同色种子边界和跨颜色边界合并到一次扫描中，并按彩图区域像素数精确预留成员空间；结构结果不变，三轮 C++ 源处理 p95 降至 `88.97/83.84/91.64ms`，本次静态短门禁全部低于100ms。完整 Python 影子供包链 p95 仍为 `1.017/1.022/1.057s`，因此尚未达到生产实时供包要求，也未验证动态场景。实验开关为 `--disable-global-missing-depth-merge`，默认深度主导、默认全局合并行为和 `candidate_0030` 均保持不变。

同一真实材料的完整静态 `600x3` 长门禁也已通过：非触边闭合候选 IoU p50=`99.3139%`、中心位移 p95=`1.8028px`、静态生命周期事件=0，三轮规范化确定性和重建一致；C++ 源处理 p95=`92.30/89.15/84.44ms`。但 p99 前两轮仍为 `104.97/103.10ms`，三轮分别有 `12/10/3` 帧超过100ms，完整 Python 影子链 p95=`1.189/1.088/1.077s`。因此长门禁只关闭静态结构和现有 p95 要求，不关闭尖峰、动态跟踪、C++ 实时供包整合或生产晋级要求。

影子链新增端到端分段计时，并消除转换器在完整源包验证后重复读取同一材料的开销；形状指纹改为与原分箱定义逐尺寸等价的向量化实现。30帧诊断中转换 p95 从约903ms降至783ms，但进一步拆分显示源包独立验证本身 p50约564ms，仍是主要成本。该验证包含材料哈希、全像素账本、轮廓层级和标定重投影，不能为追求表面实时性而删除；生产路线应由C++直接生成簇包，并把完整独立审计放到异步或抽样门禁。

真实 `T3_local_motion` 已新采60帧完整 `PCS.RawSequence/1`，输入清单 SHA256 为 `655c498113c7fc2de41d032acd5c1cde4bc8bc2a5cb9fa1acb6b4e39571925d5`。内容门禁得到 active/local pair=`72.8814%`、motion p95=`6.9987%`、motion bbox p95=`18.6699%`；6张叠加图人工确认是固定相机下手臂/手部的局部进入、移动和离开。首次控制复验在一个半像素舍入临界样本上被独立验证器误拒绝：60帧全量复算的最大分量残差为`0.5001059px`，仅1个样本超过旧`0.5001px`阈值。验证器现将允许范围固定为`0.5005px`并报告实际最大残差和临界样本数；该余量小于`0.501px`，不接受一像素错投。相同原始序列随后得到扫描60/60、跟踪结果59、观察材料哈希通过。新增无损轮廓与运动像素重叠诊断后，43个活动帧有31帧命中非大范围触边候选；全阶段涉及9个跟踪号，主导号覆盖46.51%，但19个 `Active` 命中样本中主导号`47`占84.21%。叠加图确认所选轮廓对应手部，换号主要集中在发现、快速生长和退出阶段；目标级门槛仍需冻结。因此当前只证明真实局部运动下协议和供包链连续，T3目标级稳定性尚未通过；T7也仍未退出。

使用同一60帧快照离线比较未确认候选最大关联代价 `200k/225k/250k/300k`：全阶段所选跟踪号种类为`9/8/6/6`，但 `Active` 主导号占比为`84.21/84.21/71.43/71.43%`。单纯放宽门槛没有改善已激活阶段，250k以上反而更差，因此默认200k保持不变。下一候选应针对目标进入时轮廓单调增长建立受限关联条件，并用独立T3留出片段验收，不能在本开发片段上同时调参和宣布通过。
...
2026-09-09 后续增加 `PixelClusterSensor --probe-startup`，复用新外设的Source读取原始RGBD，采集期间不运行分割、配准、补全或PNG写盘，关闭相机后再保存完整原始帧。默认3次打开、每次120帧；`--frames=2..300`、`--sessions=1..5`、必填 `--output-root=新目录`，单会话原图缓冲上限512MiB、采集时间预算20秒。默认供包流程仅增加只读逐流元数据；50ms配对门槛、分簇和补全配置、预热未证明稳定标志均不改变，不修改系统注册表或驱动。

`PixelClusterSensor/analyze_startup.py` 按预先固定的15对帧窗口，分别检查源连续性、全帧/4x4分区有效比例、孔洞翻转、相邻深度差和颜色变化；输出只是离线“变化较小候选”，不升级为传感器精度保证。三次实拍360帧保存及SHA256读回完成，但全部未通过该联合判据：首帧缺测约68.5%-69.8%，末30帧中位数约31%；0.3-1m、1-2m、2-3.5m的相邻深度差p95后段中位数约7、36-37、72-73mm。它们是帧间差，不是绝对测距误差；场景与前轮不同，不能把缺测率下降称为算法改善。当前驱动缺少曝光/增益/曝光时间逐帧元数据，也不能确认曝光稳定。验证命令、阈值、未通过原因和下一步见 [启动原始帧验证报告](docs/codex_analysis/pixel_cluster_sensor_startup_20260909.md)。

### 深度时域估计离线验证

2026-09-09 增加 `PixelClusterSensor/temporal_estimator.py` 和反例测试，比较3/5/9帧平均、中位数、带变化门禁中位数。仅在原始深度像面生成独立估计层，正常供包、成像参数、分割与本帧深度均不改变。使用18帧不重叠单元、相同输出时刻和共同有效像素集合，记录样本数、拒绝原因、支持样本年龄及边界/内部统计；不把降低方差当成距离更准确。

旧三段360帧与新采120帧分别复验。新片段中同一批像素原始对照深度差p95约52mm，3/5/9帧平均后约29.7/23.4/17.4mm；共同有效集合只占全图65.8%，不代表缺测区域改善，9帧最旧样本约266.7ms。已知真值反例证明普通平均/中位数会混入旧表面，门禁中位数也可能在缓慢移动时滞后，因此全部保持离线探针，不新增best、不启用实时滤波。命令与数据口径见 [时域估计验证报告](docs/codex_analysis/pixel_cluster_sensor_temporal_20260909.md)。

### 静态信息累积验证

2026-09-09 按新的效率方向增加 `PixelClusterSensor/static_accumulator.py`：不再扫描N帧求平均，而用有界像素账本逐帧更新，当前有深度就修正，当前缺测才有条件保留历史。默认至少3次相容观测、历史年龄不超过200ms；变化、源缺口和局部冲突触发失效。历史不自行增加支持或延长寿命，当前原始数据/彩图保留，输出显式区分历史候选与当前观测。

原360帧及另一段120帧回放各顺序重复3次；后段缺测状态翻转中位数分别从1.575%/1.630%降到0.527%/0.599%，但当前有效深度波动不变。复验材料的Python累积更新p95约13.4-13.6ms，上一版3帧平均约17.7-17.8ms；不是全系统帧耗时，不代表真实观察质量门禁通过。原始深度像面的离线探针不接入正常供包或修改best。使用方式、内存/失效边界、同色缺深度反例和读回证据见 [静态累积验证报告](docs/codex_analysis/pixel_cluster_sensor_accumulation_20260909.md)。

### 累积法动态回放验证

2026-09-09 用旧慢速平移180帧、手部遮挡重现120帧及车载连续120帧验证，累积算法和阈值未改。前30帧之后整帧颜色重置比例分别为99.33%/93.33%/100%；全部420帧历史复用为0，累积图实际退回本帧，未获得缺测减闪收益。零残留不能当成动态复用安全通过。

新增独立旧格式入口 `PixelClusterSensor/evaluate_dynamic_accumulation.py`，不会把旧彩图配准毫米深度伪造成原始相机协议。室内数据缺时间戳，必须显式 `--nominal-fps 30` 作条件测试；车载读取原数值时间戳但仍缺时间域。每段顺序重复3次、逐帧检查来源并比较输出哈希。不接入正常供包、不晋级best；下一步应区分局部运动失效与相机运动重投影。结果、命令和证据边界见 [动态累积测试](docs/codex_analysis/pixel_cluster_sensor_dynamic_accumulation_20260909.md)。
...
2026-08-01 新增驾驶有限识别首轮消融链。`--driving-risk-roi` 使用参数化分片梯形近似图像空间驾驶走廊，只约束彩图轮廓语义刷新，不裁掉全画面深度、空间簇和像素账本；差异 ROI 与走廊求交后处理，纯 motion 变化完全位于走廊外时只跳过彩图语义刷新并保留空间链。profile、converter、score 和 leaderboard 已贯通风险走廊面积、实际语义处理像素/比例、走廊外变化跳过和全帧等价工作量。`candidate_0050-0053` 分别用于全画面、固定走廊、差异驱动走廊和分级异步刷新消融；绝对耗时继续报告，但首轮机制结论以空间账本代理、时序代理和相对语义工作量为主。三个连续车载窗口的首轮 12 条运行中，0050/0051/0052/0053 平均语义工作量分别为 100%/39%/2.291%/0.385%，coverage 均为 100%、unknown 均为 0、已有 contour-lost/merge/split 计数均为 0；0051 一次全阶段系统尖峰导致 2/3 hard pass，但原配置复跑恢复为 p95 60.734ms。当前走廊仍是参数注入式机制，不是车速、转角或规划轨迹事实，0052/0053 保持 probe-only；完整目标、限制和结果见 `docs/DRIVING_RISK_ROI_ABLATION.md`。
...
菜单、场景名称、操作要求和输入提示均使用中文。输入 `1-9` 分别录制静止、缓慢直行、转弯、正常路面振动、停车遮挡重现、停车新占据、远近切换、自然光照切换和综合路线；每段达到对应固定帧数后自动停止并返回菜单，输入 `q` 退出。编号5和6只能在车辆停稳时由乘客操作目标，驾驶员不得参与。

需要保存未经分割、轮廓提取、深度滤波、拼图或叠加的原始传感器材料时，使用 librealsense 原生 `.bag` 直录入口：
